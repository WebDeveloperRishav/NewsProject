        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="post-container">
                    <?php
                    $pagesql = "SELECT * from category WHERE category_id = {$catid}";
                    ...
                    if (mysqli_num_rows($pagequery) > 0) {
                        $total_recoard = $data['post'];

                        $total_page = ceil($total_recoard / $limit);
                        echo "<ul class='pagination'>";
                        if ($page > 1) {
                            echo "<li><a href='category.php?page=" . ($page - 1) . "&id={$catid}'>Prev</a></li>";
                        }
                        if ($total_recoard > 3) {
                            for ($i = 1; $i <= $total_page; $i++) {
                                ...
                                echo "<li class='{$active}'><a href='category.php?page={$i}&id={$catid}'>{$i}</a></li>";
                            }
                        }
                        if ($total_page > $page) {
                            echo "<li><a href='category.php?page=" . ($page + 1) . "&id={$catid}'>Next</a></li>";
                        }
                        echo "</ul>";
                    }
